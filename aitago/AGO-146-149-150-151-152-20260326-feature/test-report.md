...
**#1 AGO-146 — 1-05｜上傳超大量資料 CSV（> 10,000 筆）**
- **嚴重度**：P2
- **測試備註**：`line_users count must be less than 10000`
- **問題說明**：系統對 CSV 上傳筆數有 10,000 筆上限，超過時回傳錯誤，但錯誤訊息為原始 API 回應，未轉譯為使用者可讀的提示文字
- **截圖**（需連 WiFi ai360-4f 查看）：![AGO-146-1-05](./screenshots/AGO-146-1-05.png)

---

**#2 AGO-146 — 3-05｜已有標籤的會員再次貼標不重複**
- **嚴重度**：P2
- **測試備註**：該標籤該成員已貼過、但仍顯示套用標籤數 1
- **問題說明**：對已掛有目標標籤的會員執行批次貼標，預期應跳過並反映在統計數值（matched 但不重複貼），實際統計數值不正確仍計入
- **截圖**（需連 WiFi ai360-4f 查看）：![AGO-146-3-05](./screenshots/AGO-146-3-05.png)

---

**#3 AGO-150 — 1-03｜輸入不合規 Email 格式**
- **嚴重度**：P1
...
- **截圖**（需連 WiFi ai360-4f 查看）：![AGO-150-1-04](./screenshots/AGO-150-1-04.png)

---

**#5 AGO-150 — 1-05｜輸入超長 Email 字串**
- **嚴重度**：P2
- **測試備註**：顯示後端回傳錯誤，訊息沒有修飾或正確提示用戶
- **問題說明**：輸入超過 255 字元時後端回傳錯誤，前端直接顯示原始錯誤訊息，無長度上限提示或截斷處理
- **截圖**（需連 WiFi ai360-4f 查看）：![AGO-150-1-05](./screenshots/AGO-150-1-05.png)

---

**#6 AGO-151 — 1-03｜生成 1 張圖後剩餘點數扣 3 點**
- **嚴重度**：P1
- **測試備註**：有機率發生生圖成功但圖片沒有出現的情況
- **問題說明**：AI 生圖執行後有機率圖片不顯示（但點數仍被扣除），非 100% 必現，屬間歇性失敗，建議 RD 確認 GCS 上傳或回傳邏輯
- **截圖**（需連 WiFi ai360-4f 查看）：![AGO-151-1-03](./screenshots/AGO-151-1-03.png)

---

**#7 AGO-151 — 2-04｜點數耗盡時繞過前端強制觸發 API**
- **嚴重度**：P1
- **問題說明**：當點數為 0 時，透過直接呼叫 API 繞過前端 disabled 限制，後端未進行點數守衛驗證，應在 API 層拒絕並回傳對應錯誤碼，前端需顯示錯誤提示

---

**#8 AGO-152 — 3-01｜點擊下載按鈕圖片正確下載**
- **嚴重度**：P1
- **測試備註**：影片訊息無法正確顯示
- **問題說明**：群發訊息類型為影片時，浮動面板無法正確顯示影片內容；下載功能亦異常，建議確認影片類型的預覽與下載邏輯是否有另行處理
- **截圖**（需連 WiFi ai360-4f 查看）：![AGO-152-3-01](./screenshots/AGO-152-3-01.png)

---

**#9 AGO-152 — 3-02｜圖片下載時 API 失敗**
- **嚴重度**：P2
- **測試備註**：斷網仍可下載，但會下載到只有背景的畫面
- **問題說明**：在網路異常情況下，下載動作仍觸發成功但產出的檔案僅含背景，無實際圖片內容；前端對下載失敗無錯誤提示
- **截圖**（需連 WiFi ai360-4f 查看）：![AGO-152-3-02](./screenshots/AGO-152-3-02.png)
...
| # | 工單 | 案例 | 阻擋原因 |
|---|------|------|---------|
| B1 | AGO-146 | 2-04 新標籤名稱輸入已存在的名稱 | 目前新增標籤不會提示重複、且貼標功能異常，無法確認此行為 |
| B2 | AGO-146 | 2-07 新標籤名稱輸入超長字串 | 目前新增標籤功能失敗，前置條件不滿足，無法執行 |
| B3 | AGO-149 | 4-04 選「待轉接」列表僅顯示待轉接對話 | 目前沒有「待轉接」功能，暫不測試 |
| B4 | AGO-149 | 8-03 點擊標題文字重置所有篩選條件 | 測試環境無法觸發重置行為，待確認觸發條件 |
| B5 | AGO-149 | 9-02 負責人帳號列表為空時的邊界處理 | 測試環境無法模擬無客服帳號的空環境 |
| B6 | AGO-151 | 3-01 確認月重置機制存在 | 暫不測試，需與 RD 確認排程設定後補 |
