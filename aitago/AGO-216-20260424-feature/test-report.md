...
| **總計** | | | **1586**（+rerun ~5332 次）| **Phase A 真後端失敗 6.4%（僅 AGO-228）/ Phase B 0% / Phase C 真後端失敗 13.5%（僅 AGO-229/230）** | ⚠️ 5 Bug 待 RD |

> **數字解讀說明**（避免誤解）：
> - 「**真後端失敗率**」= 扣除 QA script/fixture 自身議題後，**產品本身壞掉的比例**
> - Phase C 含 script 未修項的**總請求失敗數比**為 25.5%（452/1775），但其中 11.9% 是 QA 自己 script 層議題（空 tags 陣列 / Stage 2 未修）、13.5% 才是真 Bug（AGO-229/230）
> - RD 需處理的只有 5 個 Bug，不是 25.5% 的系統問題
...
**Phase C 總失敗數比**（Stage 1B rerun，含 retry）：452 / 1775 = **25.5%**。但需拆開看：

| 類別 | 筆數 | 佔總請求 | 說明 |
|------|----:|:------:|------|
| 🚨 **真後端 Bug**（AGO-229 / AGO-230 各 120 筆） | **240** | **13.5%** | **產品實際問題**，須 RD 修 |
| script 空陣列議題（tagging_tag restore × 120）| 120 | 6.8% | QA script 端限制，非 bug |
| Stage 2 未修 script（lock / cancel / prizes × 90）| 90 | 5.1% | 有副作用或需 fixture，保留下輪 |
| 偶發零星 | 2 | 0.1% | 可忽略 |

⚠️ **結論：真正穩定重現的後端 500 只有 2 條 endpoint（AGO-229 / AGO-230），佔 13.5%**；其餘 11.9% 屬 QA script 未修完的議題，與產品品質無關。

### 歷史輪次比較（總失敗數比）

| 輪次 | 失敗數 | 總失敗數比 | 註記 |
|------|------:|:----:|------|
| 原 2026-04-23 | 604 | 34% | 含 script 405/422 大量誤判 |
| Stage 1A | 572 | 32% | 刪 `PUT /tags/{id}`、`/coupons/{id}` 改 POST |
| **Stage 1B**（最新）| **452** | **25.5%** | 再修 4 類 script，**真後端 bug 穩定在 13.5%** |
...
| **Stage 1B**（最新）| **452** | **25.5%**（真後端 13.5% + script 議題 11.9%）| 加 `source.id`、`line_user_ids`、更新 coupon 移 `is_unique_code`、notes 改 ULID |
...
| [AGO-226](https://ai360c.atlassian.net/browse/AGO-226) | `GET /line_users/coupons` | p95 ≈ 7.5s + **公開**（無 auth）+ 無 cache + DoS 風險 | **Highest** |
...
| 公開 endpoint（LIFF / token，共 9 條，本次測 7 條） | < 500ms | < 1 秒 | 6 條達標（≤ 95ms）/ **1 條嚴重超標（AGO-226 p95 ≈ 7.5 秒）** |
...
### 公開端點（無 auth）完整清單（OpenAPI `security: []` 查證）

依 `https://feature-line-crm.aitago.tw/docs/api.json` 確認，全 API 共 **9 條明確公開端點**：

| # | Endpoint | 本次測試 | p95 | 備註 |
|---|----------|:-------:|:---:|------|
| 1 | `POST /token` | ✅ Phase B | <150ms | 取 JWT |
| 2 | `POST /token/refresh` | ✅ Phase B | <150ms | |
| 3 | `POST /token/revoke` | ✅ Phase B | <150ms | |
| 4 | `GET /locales` | ✅ Phase A | 78ms | |
| 5 | `GET /liff/apps` | ✅ Phase A | 77ms | |
| 6 | `GET /line_users/coupons` | ✅ Phase A | **7469ms** | 🚨 AGO-226 慢 + DoS 風險 |
| 7 | `GET /line_users/{lineUserId}/coupons` | ⚠️ 測了但 script 誤標 `auth:true`（仍量到 p95=95ms）| 95ms | Script 應改 `auth:false` |
| 8 | `GET /line_users/{lineUserId}/coupons/{couponCode}` | ❌ **未測**（需 fixture coupon code）| — | 下輪補 |
| 9 | `GET /line_users/{lineUserId}/user_coupon_codes/{userCouponCodeId}` | ❌ **未測**（需 fixture user_coupon_code id）| — | 下輪補 |

⚠️ **AGO-226 公開端點的風險放大**：由於無 auth 保護，任何人都能對此 endpoint 打量，7.5 秒 p95 可被利用做 DoS 攻擊；DB 壓力也會隨外部流量不受控。

#### #7 / #8 / #9 詳細說明

**這 3 條都不是產品問題**，是「本輪測試腳本覆蓋度未覆蓋到」，原因各自不同：

**#7 `GET /line_users/{lineUserId}/coupons`（script 誤標 auth:true）**
- 建 k6 腳本時從名稱看「帶 uid」判斷為後台管理用（誤認為需 JWT），實際 OpenAPI 確認為公開端點（應給 LIFF 前端查某人自己的券用）
- 後端對「公開 endpoint 有帶 auth header」不會擋（auth 可有可無），**p95=95ms 數字仍有效，只是 SLO 分類錯誤**
- 修正成本：改 1 行 `auth: false` + 重跑 Phase A（~35 分鐘）；效益低（數字已知、歸類對即可）
- 下輪補修

**#8 `GET /line_users/{lineUserId}/coupons/{couponCode}`（無 fixture 未測）**
- 需帶有效的 `couponCode`（某 user 已領的券代碼字串）
- `helpers/fetch-fixtures.mjs` 目前沒撈「已領 user-coupon-code」類資料
- 若要測需先：assign 一張券給測試 user → 取得 code → 帶 path 測
- 用途：LIFF 頁用戶點某張票券看詳情的 API
- **沒測損失**：此條 read 端點效能未知，但不在慢 endpoint 風險群（結構單純）

**#9 `GET /line_users/{lineUserId}/user_coupon_codes/{userCouponCodeId}`（無 fixture 未測）**
- 需帶 `user_coupon_code` 的**內部 ULID**（`01kp...`），跟 #8 的 code 字串不同
- 同樣 fixture 沒撈到
- 技術上只是 `SELECT FROM user_coupon_codes WHERE id = ?` 的 wrapper，**預期不會慢**

**總評估**：3 條都屬「**測試覆蓋度補強項**」，不影響本輪對 AGO-225/226/228/229/230 共 5 個真 Bug 的判定。修正成本見 §8 下一步「下輪（兩週內）」。
...
3. ~~QA 修 script：`PUT /tags/{id}` + `PUT /coupons/{id}` 改 PATCH~~ → **已於 2026-04-24 Stage 1A 處理**（OpenAPI 對照後改為：刪 `PUT /tags/{id}` + `/coupons/{id}` 改 POST）

### 下輪（兩週內）
1. `/materials` p95 2038ms vs 423ms 差異 — 跑第三輪 confirm
2. `/campaigns/{id}/insights` p95 1.3 秒 — 聚合 cache 評估
3. **補測 2 條未測公開端點**：`GET /line_users/{lineUserId}/coupons/{couponCode}` + `GET /line_users/{lineUserId}/user_coupon_codes/{userCouponCodeId}`（需先備 fixture）
4. **Script 修正**：`GET /line_users/{uid}/coupons` 從 `auth:true` 改 `auth:false`（實際是公開端點）
5. Phase C Stage 2：`POST /prizes`（補 activity_playground_id fixture）、`/coupons/codes/lock` + `cancel`（需評估副作用）
6. `tagging_tag` 空陣列 restore 邏輯改寫（避免 422）
